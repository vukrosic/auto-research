...
class Hyperparameters:
    ...
    mlp_act = os.environ.get("MLP_ACT", "leaky05_gradfloor")
    ...
    embed_bottleneck = int(os.environ.get("EMBED_BOTTLENECK", "128"))
...
def main() -> None:
    ...
    debug_val_tokens = int(os.environ.get("DEBUG_VAL_TOKENS", "0"))
    if debug_val_tokens > 0:
        usable = ((min(debug_val_tokens, val_tokens.numel() - 1)) // args.train_seq_len) * args.train_seq_len
        val_tokens = val_tokens[: usable + 1]
        log0(f"DEBUG_VAL_TOKENS: truncated val set to {val_tokens.numel() - 1} tokens")
    ...
    while True:
        ...
        should_validate = (last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0)) and not (step == 0 and args.val_loss_every > args.iterations)
        if should_validate:
            torch.cuda.synchronize()

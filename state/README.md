...
- Experiment status lives in `experiments/snapshots/*/status`
- Experiment metadata lives in `experiments/snapshots/*/meta.json`
- Frontier record lives in `experiments/current_best.json`

If a state file disagrees with the primary records, the primary records win. Regenerate the state file.
...
- [Frontier](/root/research/autoresearch/state/FRONTIER.md) — derived from `experiments/current_best.json`
